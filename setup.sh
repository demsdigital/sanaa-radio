...
# ==================== news/[id]/page.tsx ====================
cat > "src/app/news/[id]/page.tsx" << 'EOF'
import { db } from "@/db";
...
echo "✅ programs/page.tsx — SEO"

# ==================== SEO: program slug ====================
grep -q 'generateMetadata' "src/app/programs/[slug]/page.tsx" || \
sed -i '' 's/type Props = { params: Promise<{ slug: string }> };/import type { Metadata } from "next";\n\ntype Props = { params: Promise<{ slug: string }> };\n\nexport async function generateMetadata({ params }: Props): Promise<Metadata> {\n  const { slug } = await params;\n  const [program] = await db.select().from(programs).where(eq(programs.slug, slug));\n  if (!program) return { title: "برنامج غير موجود | إذاعة الجمهورية اليمنية" };\n  return {\n    title: `${program.name} | إذاعة الجمهورية اليمنية`,\n    description: program.description ?? `استمع لحلقات برنامج ${program.name} على إذاعة الجمهورية اليمنية.`,\n    openGraph: { title: `${program.name} | إذاعة الجمهورية اليمنية`, locale: "ar_YE", type: "website" },\n  };\n}/' "src/app/programs/[slug]/page.tsx"
echo "✅ programs/[slug]/page.tsx — SEO"

# ==================== SEO: homepage ====================
grep -q 'export const metadata' src/app/page.tsx || \
sed -i '' 's/export default async function HomePage/import type { Metadata } from "next";\n\nexport const metadata: Metadata = {\n  title: "إذاعة الجمهورية اليمنية — البرنامج العام",\n  description: "إذاعة الجمهورية اليمنية — الصوت الحقيقي منذ عقود.",\n  openGraph: { title: "إذاعة الجمهورية اليمنية — البرنامج العام", description: "الصوت الحقيقي منذ عقود.", locale: "ar_YE", type: "website" },\n};\n\nexport default async function HomePage/' src/app/page.tsx
echo "✅ page.tsx — SEO"

# ==================== كروت الأخبار → روابط ====================
# استبدال div الخبر بـ Link
python3 - << 'PYEOF'
import re
...
old = '''              <div key={item.id} className="bg-white border border-slate-200 rounded-xl p-5 hover:border-blue-200 hover:shadow-md transition-all">
...
new = '''              <Link key={item.id} href={`/news/${item.id}`} className="bg-white border border-slate-200 rounded-xl p-5 hover:border-blue-200 hover:shadow-md transition-all group block">
                <div className="text-slate-400 text-xs mb-2">{new Date(item.publishedAt).toLocaleDateString("ar-YE")}</div>
...
if old in content:
    ...
    with open("src/app/page.tsx", "w") as f:
        f.write(content)
    print("✅ كروت الأخبار أصبحت روابط")
else:
    print("⚠️  لم يتم العثور على كروت الأخبار — تحقق يدوياً")
PYEOF

echo ""
echo "✅ انتهى! شغّل الآن:"
echo "git add . && git commit -m 'feat: صفحة خبر مفصل + SEO لكل الصفحات' && git push origin main"
