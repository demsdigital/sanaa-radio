...
export default function DayTabs({ counts, today }: { counts: Record<string,number>; today: string }) {
  ...
  return (
    <div className="flex flex-wrap gap-2 mb-8 sm:grid sm:grid-cols-7">
      {days.map((d) => {
        const isSelected = d.value === selected;
        const isToday = d.value === today;
        return (
          <button key={d.value} onClick={() => router.push(`/schedule?day=${d.value}`)}
            className={`rounded-xl p-3 text-center border transition-all w-[calc(25%-6px)] sm:w-auto ${isSelected ? "bg-blue-600 border-blue-500 text-white shadow-lg" : isToday ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-white border-slate-200 text-slate-700 hover:border-blue-200"}`}>
