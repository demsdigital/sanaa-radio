    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{episodes.length} حلقة</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-right text-slate-500 text-xs font-medium px-6 py-4">الحلقة</th>
                <th className="text-right text-slate-500 text-xs font-medium px-6 py-4">البرنامج</th>
                <th className="text-right text-slate-500 text-xs font-medium px-6 py-4">المدة</th>
                <th className="text-right text-slate-500 text-xs font-medium px-6 py-4">صوت</th>
              ...
              {episodes.map((ep) => (
                <tr key={ep.id} className="border-b border-slate-100 hover:bg-white/2">
                  <td className="px-6 py-4">
                    ...
                    <div className="text-slate-400 text-xs mt-1">{new Date(ep.publishedAt).toLocaleDateString("ar-YE")}</div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-gray-300 text-sm">{getProgramName(ep.programId)}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-slate-500 text-sm">{formatDuration(ep.duration)}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`text-xs px-2 py-1 rounded ${ep.audioUrl ? "bg-green-500/20 text-green-400" : "bg-gray-500/20 text-slate-400"}`}>
                      {ep.audioUrl ? "✓ موجود" : "لا يوجد"}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => openEdit(ep)} className="text-slate-500 hover:text-slate-900 text-xs px-3 py-1 border border-slate-200 rounded hover:border-white/30 transition-colors">تعديل</button>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-slate-200 rounded-xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-slate-500 text-sm mb-2">البرنامج</label>
              ...
              <div>
                <label className="block text-slate-500 text-sm mb-2">عنوان الحلقة</label>
              ...
              <div>
                <label className="block text-slate-500 text-sm mb-2">الوصف</label>
              ...
              <div>
                <label className="block text-slate-500 text-sm mb-2">الملف الصوتي</label>
                <input type="file" accept="audio/*" onChange={(e) => e.target.files?.[0] && handleUpload(e.target.files[0])} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-slate-500 text-sm" />
              ...
              <div>
                <label className="block text-slate-500 text-sm mb-2">المدة (بالثواني)</label>
