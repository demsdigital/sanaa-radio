    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-600 text-sm mt-1">{items.length} برنامج في الجدول</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {days.map(d => (
          <button key={d.value} onClick={() => setSelectedDay(d.value)}
            className={`px-4 py-2 rounded-lg text-sm transition-colors ${selectedDay === d.value ? "bg-blue-600 text-slate-900 font-bold" : "bg-white text-slate-700 border border-slate-200 hover:text-slate-900"}`}>
            {d.label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-slate-600 text-center py-20">لا يوجد جدول لهذا اليوم</div>
      ) : (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="text-right text-slate-700 text-xs font-medium px-6 py-4">الوقت</th>
                <th className="text-right text-slate-700 text-xs font-medium px-6 py-4">البرنامج</th>
                <th className="text-right text-slate-700 text-xs font-medium px-6 py-4">النوع</th>
            ...
            <tbody>
              {filtered.sort((a, b) => a.timeStart.localeCompare(b.timeStart)).map((item) => (
                <tr key={item.id} className="border-b border-slate-100 hover:bg-white/2">
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs px-2 py-1 rounded ${item.type === "live" ? "bg-red-500/20 text-red-500" : "bg-gray-500/20 text-slate-700"}`}>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-slate-200 rounded-xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-slate-700 text-sm mb-2">اسم البرنامج</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 text-sm mb-2">من</label>
                ...
                <div>
                  <label className="block text-slate-700 text-sm mb-2">إلى</label>
              ...
              <div>
                <label className="block text-slate-700 text-sm mb-2">اليوم</label>
              ...
              <div>
                <label className="block text-slate-700 text-sm mb-2">النوع</label>
