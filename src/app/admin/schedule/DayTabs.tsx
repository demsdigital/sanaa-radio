...
export default function DayTabs({ counts, today }: Props) {
  ...
  return (
    <div className="grid grid-cols-7 gap-2 mb-8">
      {days.map((d) => {
        ...
        return (
          <button key={d.value}
            onClick={() => router.push(`/schedule?day=${d.value}`)}
            ...
            {isToday && !isSelected && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 mx-auto mt-1" />}
            {isSelected && <div className="w-1.5 h-1.5 rounded-full bg-white mx-auto mt-1 animate-pulse" />}
          </button>
        );
      })}
    </div>
